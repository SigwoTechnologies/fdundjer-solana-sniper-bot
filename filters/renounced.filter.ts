...
export class RenouncedFreezeFilter implements Filter {
  ...
  async execute(poolKeys: LiquidityPoolKeysV4): Promise<FilterResult> {
    ...
    try {
      ...
      const freezable = this.checkFreezable ? deserialize.freezeAuthorityOption !== 0 : false;
      ...
      const result = {
        ok: ok,
        message: ok ? `RenouncedFreeze -> Passed` : `RenouncedFreeze -> Creator can ${message.join(' and ')} tokens`,
      };
